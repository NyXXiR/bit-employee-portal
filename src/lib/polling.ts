 ...
 * 적었으나, 503 응답 509건 중 Retry-After 헤더가 있는 것은 0건이었다. 값은 본문에만
 * 실려 오고(303건), 전부 상수 30이다. 헤더만 읽는 구현은 조용히 null 을 받는다 —
 * 명세가 헤더와 본문 필드를 둘 다 정의해 둔 것으로 보아 의도된 함정으로 보인다.
 * externalRetryAfterSeconds 가 헤더 -> 본문 순으로 둘 다 읽는 이유다.
 *
 * 게다가 retryAfter 는 1초 안에 돌아온 503 에만 붙고(303건 전부, p50 32.8ms),
 * 30초를 태운 503(206건 전부)에는 없다. 의미가 뒤집혀 있다.
